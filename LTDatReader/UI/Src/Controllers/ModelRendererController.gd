...
func on_file_load(path):
	...
	# Duplizierte Referenzen auf das alte Objekt mitleeren, sonst haelt
	# self.loaded_file.scene/raw_data bis zum naechsten Laden eine tote
	# Referenz auf die gerade freigegebene Node - vermutlich Ursache der
	# "ObjectDB instances leaked at exit"-Meldung beim Programmende.
	self.loaded_file.scene = null
	self.loaded_file.raw_data = null
...
func _threaded_load(path):
	...
	# WorldBuilder.build() kann bei unbekanntem Format auch FAILED (=1, eine
	# Ganzzahl) zurueckgeben statt null - deshalb hier beides abfangen, nicht
	# nur null. Sonst rutscht der Fehlerfall bis zum "ERROR: Scene loading
	# failed"-Zweig durch und der Mutex/loading-Flag-Fix dort greift zwar,
	# aber sauberer ist es, den erwarteten Fehlerfall schon hier zu fangen.
	if scene == null or typeof(scene) == TYPE_INT:
