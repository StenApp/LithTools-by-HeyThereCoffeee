...
var cheat_skeleton = null  # lazy statt eager Skeleton.new() - wird bei jedem Modell-Laden sonst verwaist
...
func build(source_file, options):
	...
	# Clean up! scene.pack() kopiert root in die PackedScene-Resource, nimmt
	# root aber nicht in Besitz - ohne free() bleibt der gesamte Node-Baum
	# (Skeleton, alle MeshInstances, AnimPlayer) als Leiche im Speicher.
	root.free()
