...
var cheat_skeleton = null  # lazy statt eager Skeleton.new() - siehe Fix bei der Zuweisung unten
...
func build(source_file, options):
	...
	# Clean up! (frueher stand hier "root.queue_free()", auskommentiert - das
	# scheiterte vermutlich, weil root nie in einer SceneTree haengt und
	# queue_free() intern get_tree() braucht. free() laeuft synchron ohne
	# SceneTree-Abhaengigkeit und sollte hier sicher sein.)
	root.free()
