	...
	func write(model, path, version, missing_textures = [], missing_tex_path = ""):
		...
		for world_model in model.world_models:
			...
			for poly in world_model.polies:
				...
				# Bugfix: "O" in poly und poly.O != null waren beide wirkungslos -
				# WorldPoly.O ist IMMER als Feld deklariert (also "in poly" immer wahr),
				# und ein Vector3 ist in GDScript nie gleich null (also != null immer
				# wahr) - der else-Zweig wurde dadurch nie erreicht. Fuer LT1 wird
				# poly.O/P/Q beim Einlesen nie gesetzt (bleibt (0,0,0)!) - da muss
				# echt auf surface.O/P/Q zurueckgefallen werden. Fuer LT2/PS2 ist
				# poly.O ohnehin nur eine Kopie von surface.O (identischer Wert),
				# fuer Talon hat poly seine eigene, echte OPQ - beide Faelle sind
				# mit poly.O also unproblematisch.
				if model.has_method("is_lithtech_1") and model.is_lithtech_1():
					texture_info_node.create_property([surface.O.x, surface.O.y, surface.O.z])
					texture_info_node.create_property([surface.P.x, surface.P.y, surface.P.z])
					texture_info_node.create_property([surface.Q.x, surface.Q.y, surface.Q.z])
				else:
					texture_info_node.create_property([poly.O.x, poly.O.y, poly.O.z])
					texture_info_node.create_property([poly.P.x, poly.P.y, poly.P.z])
					texture_info_node.create_property([poly.Q.x, poly.Q.y, poly.Q.z])
