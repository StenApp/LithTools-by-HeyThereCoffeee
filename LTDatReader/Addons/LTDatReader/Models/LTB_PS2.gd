...
#		f.seek(world_model_pos)
	...
	func read_quat(file : File):
		# Bugfix: war W,X,Y,Z, muss X,Y,Z,W sein - verifiziert gegen LTB_PS2_Level_V66_PS2.bt
		# ("struct LTQuaternion { float x, y, z, w; }")
	...
	class WorldSurface:
		...
		func read(ltb: LTB_PS2, f : File):
			self.O = ltb.read_vector3(f)
			self.P = ltb.read_vector3(f)
			self.Q = ltb.read_vector3(f)
